...
const statusLabels: Record<ReceiptPrinterStage, ReactNode> = {
  processing: "Reading the city",
  printing: "Printing your survival",
  ...
  return (
    <ReceiptPrinterContext.Provider value={context}>
      <section
        ...
        className={cn(
          "relative isolate flex w-full max-w-[28rem] flex-col items-center",
          className,
        )}
        data-stage={stage}
        {...props}
...
function ReceiptPrinterHeader({
  ...
  return (
    <div
      className={cn(
        "relative z-20 flex h-11 items-start justify-between text-[#161412]",
        className,
      )}
      {...props}
    >
...
function ReceiptPrinterScreen({
  ...
  return (
    <div
      className={cn(
        "relative z-10 isolate overflow-hidden rounded-[var(--printer-inner-radius)] border border-[#3d3833] bg-[#1c1916] p-4 text-[#faf6ef] shadow-inner after:pointer-events-none after:absolute after:inset-0 after:z-0 after:rounded-[inherit] after:shadow-[inset_0_0_16px_2px_rgba(0,0,0,0.25)] after:content-['']",
        className,
      )}
      {...props}
    >
      <div className="relative z-10 text-[#faf6ef]">{children}</div>
    </div>
  );
}
    ...
    >
      <AnimatePresence initial={false} mode="sync">
        {isComplete ? (
          ...
          <motion.span
            animate={{ opacity: 1, transform: "scale(1)" }}
            className="col-start-1 row-start-1 grid place-items-center text-[#faf6ef]"
            exit={{
              opacity: animate ? 0 : 1,
              transform: move ? "scale(0.96)" : "scale(1)",
            }}
      ...
      >
        <AnimatePresence initial={false} mode="sync">
          <motion.div
            ...
            className="col-start-1 row-start-1 truncate text-base font-semibold leading-none text-[#faf6ef]"
            exit={{
              opacity: animate ? 0 : 1,
...
function ReceiptPrinterPaper({
  ...
  return (
    <article
      className={cn(
        "relative z-10 min-h-96 bg-[#f4efe6] bg-[url('/textures/receipt-paper.svg')] bg-cover px-7 pt-8 pb-10 font-mono text-[#1c1916] bg-blend-soft-light",
        className,
      )}
...
function ReceiptPrinterOutput({
  ...
  return (
    <div
      className={cn(
        "relative z-50 -mt-4 h-[48rem] w-[calc(88%+2rem)] max-w-full overflow-x-hidden overflow-y-auto px-4",
        className,
      )}
      {...props}
    >
